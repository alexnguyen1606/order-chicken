jQuery(function ($) {
    $(document).ready(function () {
        ...
        function init() {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    roles = res;
                    loadListUnit(res,6,'.tree-poscode-tct',true);
                    loadListUnit(res,3580,'.tree-poscode-dv',false);
                },
                error: function (res) {
                    console.log(res);
        ...
        function loadListUnit(roles,idParent,position,isOpenChild) {
            $.ajax({
                ...
                success: function (res) {
                    ...
                    $(res).each(function (index, item) {
                        ...
                        row += '<div  code_unit="' + item.unitCode + '" id="id_pers_root_for_edit_' + item.id + '" id_pers_root="' + item.id + '" class="showChildRoot carett showChildRootForEdit ">&nbsp;<i class="far fa-building text-success"></i>&nbsp;' + item.name + '</div>'
                        // if (roles.includes("TCT") || roles.includes(item.id)) {
                        //     row += '<input class="ml-2 edit-item px-4 " value="'+item.id+'" type="checkbox">'
                        // }
                        row += '</div>'
                    ...
                    if (res != null) {
                        $(res).each(function (index, items) {
                            ...
                            if (roles.includes("TCT") || roles.includes(items.id)) {
                                row += '<input class="ml-2 edit-item px-4 " value="'+items.id+'" type="checkbox">'
                            }
                            row += '</div>'
