...
jQuery(function ($) {
    $(document).ready(function () {
        function getListReported(url) {
            if (url == null || url == "") {
                url = "/api/admin/report/course/course-join/list";
            }
            var data = {};
            $.ajax({
                type: "POST",
        ...
        function paging(totalPage, currentPage) {
            $('#pagination-test').twbsPagination({
                ...
                onPageClick: function (event, page) {
                    if (currentPage != page) {
                        var url = "/api/admin/report/course/course-join/list";
                        url += "?page=" + page;
                        getListReported(url);
                    }
                }
        ...
        $('#download_report').click(function () {
            ...
            $.ajax({
                type: "PUT",
                url: "/api/admin/report/course/course-join/"+data.courseId,
                headers: {"Authorization": "Bearer " + localStorage.getItem('eln_token')},
                // data: JSON.stringify(data),
                // dataType: "json",
                contentType: "application/json",
        ...
        $('#inputReport').on('submit',function (e) {
            e.preventDefault();
           fetchCourse();
        });
    })
