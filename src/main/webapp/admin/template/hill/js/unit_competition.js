$(document).ready(function(){
    ...
    $('#btn_add_unit_competion').click(function(){
        ...
        row+='<input name="idPoscodeCompetition[]"  index="'+i+'"  type="text" id_unit="" value="" class="form-control class_box_id_unit" onkeyup="getNamePoscodeById(this)" id="text_box_name_unit_competion'+i+'" placeholder="Mã đơn vị">'
        row+='<input  type="hidden" id_unit="" name="email" class="form-control class_name_unit" id="name_unit'+i+'" placeholder="">'
        row+='<span id="name_unit">Đơn vị:<span class="class_name_unit_form" id="name_unit_form'+i+'">--Chưa có thông tin--<span></span></span></span>'
        row+='</div>'
...
function showChildRootAppend(btn) {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if(checkUnitCode(codeUnit,idPerRoot)){showPoscodeRootCompetiton(idPerRoot,name_poscode,codeUnit,i);}
