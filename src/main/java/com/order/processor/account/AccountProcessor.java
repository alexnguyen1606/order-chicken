...
//        result.and(qAccount.status.eq(1));
    ...
    public List<AccountDTO> findAll(AccountDTO accountDTO, Pageable pageable) {
        BooleanBuilder builder = commonBuilder(accountDTO);
        List<AccountDTO> result = accountService.findAll(builder, pageable).stream().map(accountMapper::toDTO).collect(Collectors.toList());
        if (!result.isEmpty()) {
    ...
    public void updatePassword(AccountDTO accountDTO) throws Exception {
        ...
        if (!passwordEncoder.matches(accountDTO.getOldPassword(),account.getPassword())) throw new Exception("Mật khẩu xác nhận k chính xác");
